...
Public Class FrmOrdersGrid
    ...
    Public Sub New()
        ...
        _Filter = New Order.Filter(DgvData)
        _Filter.Filter()
    ...
    Private Sub BtnInclude_Click(sender As Object, e As EventArgs) Handles BtnInclude.Click
        Dim Form As New FrmOrder(New Order, Me)
        Form.ShowDialog()
    End Sub
    Private Sub BtnEdit_Click(sender As Object, e As EventArgs) Handles BtnEdit.Click
        Dim Form As FrmOrder
        If DgvData.SelectedRows.Count = 1 Then
            Try
                _Order = New Order(DgvData.SelectedRows(0).Cells("ID").Value)
                Form = New FrmOrder(_Order, Me)
                _Order.GetItems(Form.DgvItems)
                Form.ShowDialog()
    ...
    Private Sub BtnDelete_Click(sender As Object, e As EventArgs) Handles BtnDelete.Click
        If DgvData.SelectedRows.Count = 1 Then
            If CMessageBox.Show("Deseja excluir o registro selecionado?", CMessageBox.CMessageBoxType.Question, CMessageBox.CMessageBoxButtons.YesNo) = DialogResult.Yes Then
                Try
                    _Order.Load((DgvData.SelectedRows(0).Cells("ID").Value))
                    Try
                        _Order.Delete()
    ...
    Private Sub DgvData_CellFormatting(sender As Object, e As DataGridViewCellFormattingEventArgs) Handles DgvData.CellFormatting
        If e.ColumnIndex = DgvData.Columns("Status").Index Then
            ...
            End Select
        End If
        If e.ColumnIndex = DgvData.Columns("Valor Bruto").Index Then e.CellStyle.Format = "N2"
        If e.ColumnIndex = DgvData.Columns("Valor Bruto").Index Then e.CellStyle.Alignment = DataGridViewContentAlignment.MiddleRight
        If e.ColumnIndex = DgvData.Columns("Valor Líquido").Index Then e.CellStyle.Format = "N2"
        If e.ColumnIndex = DgvData.Columns("Valor Líquido").Index Then e.CellStyle.Alignment = DataGridViewContentAlignment.MiddleRight
    End Sub
    ...
    Private Sub LoadDetails()
        ...
        If BtnDetails.Checked Then
            Try
                If DgvData.SelectedRows.Count = 1 Then
                    Order = New Order(DgvData.SelectedRows(0).Cells("ID").Value)
                    Order.GetItems(DgvDetails)
                Else
                    DgvDetails.DataSource = Nothing
                End If
            Catch ex As Exception
                CMessageBox.Show("ERRO0000", "Ocorreu um erro ao consultar os contatos do pedido selecionado.", CMessageBox.CMessageBoxType.Error, CMessageBox.CMessageBoxButtons.OK, ex)
            End Try
        End If
    End Sub

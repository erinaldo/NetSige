...
Public Class FrmItemGroupsGrid
    Private _Group As New ItemGroup
    Private _Filter As ItemGroup.Filter
    Public Sub New()
        InitializeComponent()
        SplitContainer1.Panel1Collapsed = True
        SplitContainer1.SplitterDistance = 250
        _Filter = New ItemGroup.Filter(DgvData)
        _Filter.Filter()
    ...
    Private Sub BtnInclude_Click(sender As Object, e As EventArgs) Handles BtnInclude.Click
        Dim Form As New FrmItemGroup(New ItemGroup, Me)
        Form.ShowDialog()
    End Sub
    Private Sub BtnEdit_Click(sender As Object, e As EventArgs) Handles BtnEdit.Click
        Dim Form As FrmItemGroup
        If DgvData.SelectedRows.Count = 1 Then
            Try
                _Group = New ItemGroup(DgvData.SelectedRows(0).Cells("ID").Value)
                Form = New FrmItemGroup(_Group, Me)
                Form.ShowDialog()
    ...
    Private Sub BtnDelete_Click(sender As Object, e As EventArgs) Handles BtnDelete.Click
        If DgvData.SelectedRows.Count = 1 Then
            If CMessageBox.Show("Deseja excluir o registro selecionado?", CMessageBox.CMessageBoxType.Question, CMessageBox.CMessageBoxButtons.YesNo) = DialogResult.Yes Then
                Try
                    _Group.Load((DgvData.SelectedRows(0).Cells("ID").Value))
                    Try
                        _Group.Delete()
